...
use crate::vaachak_x4::runtime::state_io_progress_write_backend_binding::{
    phase39a_execute_progress_write, Phase39aBackendWriteResult, Phase39aBackendWriteStatus,
    Phase39aProgressBookId, Phase39aProgressStatePath, Phase39aProgressWriteBackend,
    Phase39aProgressWriteDecision, Phase39aProgressWriteMode, Phase39aProgressWriteOutcome,
    Phase39aProgressWritePreflight, Phase39aProgressWriteReport, Phase39aProgressWriteRequest,
...
use crate::vaachak_x4::runtime::state_io_progress_write_callback_backend::{
    phase39b_execute_callback_progress_write, Phase39bCallbackBackendOutcome,
...
pub fn phase39c_execute_progress_write_dry_run(
    ...
    phase39c_report_from_phase39a(
        Phase39cProgressWriteBackendKind::None,
        phase39a_report,
    )
}
...
pub fn phase39c_execute_progress_write_with_recording_backend(
    ...
    let phase39a_report = phase39a_execute_progress_write(Some(backend), request.as_phase39a_request());
    ...
    match request.mode {
        ...
        Phase39cProgressWriteLaneMode::CommitViaRecordingBackend => {
            if let Some(backend) = recording_backend {
                ...
                let phase39a_report =
                    phase39a_execute_progress_write::<Phase39cRecordingProgressBackend>(
                        None,
                        request.as_phase39a_request(),
                    );
                phase39c_report_from_phase39a(
                    Phase39cProgressWriteBackendKind::Recording,
                    phase39a_report,

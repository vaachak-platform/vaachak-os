...
TXT_TITLEMAP_NAMES_CONFIRMED="${TXT_TITLEMAP_NAMES_CONFIRMED:-0}"
...
OUT="${OUT:-/tmp/phase40h-device-report.txt}"

status="ACCEPTED"
reason="HostTitleMapTxtDisplayNamesConfirmed"

if [ "$TXT_TITLEMAP_NAMES_CONFIRMED" != "1" ]; then
  status="REJECTED"; reason="TxtTitleMapNamesNotConfirmed"
...
{
  echo "# Phase 40H Device Report"
  echo "status=$status"
  echo "reason=$reason"
  echo "txt_titlemap_names_confirmed=$TXT_TITLEMAP_NAMES_CONFIRMED"

...
pub fn parse_sleep_image_mode(data: &[u8]) -> Option<SleepImageMode> {
    ...
    } else if eq_ascii(s, b"fast-daily")
        || eq_ascii(s, b"fast_daily")
        || eq_ascii(s, b"fastdaily")
    {
...
mod tests {
    ...
    fn sleep_image_mode_parser_accepts_supported_modes() {
        ...
        assert_eq!(parse_sleep_image_mode(b"fast-daily"), Some(SleepImageMode::FastDaily));
        assert_eq!(parse_sleep_image_mode(b"static"), Some(SleepImageMode::StaticBitmap));
        assert_eq!(parse_sleep_image_mode(b"cached"), Some(SleepImageMode::Cached));
        assert_eq!(parse_sleep_image_mode(b"text"), Some(SleepImageMode::TextFallback));
        assert_eq!(parse_sleep_image_mode(b"off"), Some(SleepImageMode::NoRedraw));
        assert_eq!(parse_sleep_image_mode(b"no-redraw"), Some(SleepImageMode::NoRedraw));
    }

...
use crate::vaachak_x4::runtime::state_io_guarded_persistent_backend_stub::phase38q_live_mutation_still_disabled;
use crate::vaachak_x4::runtime::state_io_guarded_read_before_write_stub::{
    Phase38rNextLane, phase38r_live_mutation_still_disabled,
};
use crate::vaachak_x4::runtime::state_io_guarded_write_backend_adapter_acceptance::phase38p_live_mutation_still_disabled;
use crate::vaachak_x4::runtime::state_io_guarded_write_backend_dry_run_executor::PHASE_38M_LIVE_MUTATION_ENABLED;
...
use crate::vaachak_x4::runtime::state_io_guarded_write_dry_run_acceptance::phase38n_live_mutation_still_disabled;
use crate::vaachak_x4::runtime::state_io_write_design_consolidation::phase38c_writes_enabled;
...
impl Phase38sPhase39FirstWriteScope {
    pub const fn record_kind(self) -> Phase38lStateRecordKind {
        match self {
            Self::Progress => Phase38lStateRecordKind::Progress,
            Self::Theme => Phase38lStateRecordKind::Theme,
            Self::Metadata => Phase38lStateRecordKind::Metadata,
            Self::Bookmark => Phase38lStateRecordKind::Bookmark,
            Self::BookmarkIndex => Phase38lStateRecordKind::BookmarkIndex,
        }
    }

    pub const fn allowed_intent(self) -> Phase38lMutationIntent {
        match self {
            Self::BookmarkIndex => Phase38lMutationIntent::AppendIndex,
            Self::Progress | Self::Theme | Self::Metadata | Self::Bookmark => {
    ...
    pub const fn label(self) -> &'static str {
        match self {
            Self::Progress => "progress-only",
            Self::Theme => "theme-only",
            Self::Metadata => "metadata-only",
            Self::Bookmark => "bookmark-only",
            Self::BookmarkIndex => "bookmark-index-only",
        }
    }
}
...
pub fn phase38s_write_lane_handoff_report() -> Phase38sWriteLaneHandoffReport {
    ...
    Phase38sWriteLaneHandoffReport {
        status,
        reason,
        first_write_scope: Phase38sPhase39FirstWriteScope::Progress,
...
pub fn phase38s_live_mutation_enabled_now() -> bool {
    PHASE_38S_LIVE_MUTATION_ENABLED || PHASE_38M_LIVE_MUTATION_ENABLED || phase38c_writes_enabled()
}
...
pub const fn phase38s_recommended_phase39_first_scope() -> Phase38sPhase39FirstWriteScope {
    Phase38sPhase39FirstWriteScope::Progress
}

# Documentation Index

## Current-state docs

- `README.md`
- `SCOPE.md`
- `ROADMAP.md`
- `docs/architecture/current-runtime.md`
- `docs/architecture/ownership-map.md`
- `docs/architecture/current-architecture-and-roadmap.md`

## Hardware acceptance docs

- `docs/architecture/vaachak-os-hardware-runtime-architecture.md`
- `docs/architecture/hardware-physical-full-migration-consolidation.md`
- `docs/architecture/hardware-physical-full-migration-cleanup.md`
- `docs/architecture/pulp-hardware-reference-deprecation-audit.md`
- `docs/architecture/pulp-hardware-dead-path-quarantine.md`
- `docs/architecture/pulp-hardware-dead-path-removal.md`
- `docs/architecture/vendor-pulp-os-scope-reduction.md`

## Product roadmap docs

- `docs/roadmap/x4-reader-roadmap.md`
- `docs/state/current-state-models.md`
- `docs/formats/vchk-package-contract.md`

## Development and validation docs

- `docs/development/build-and-flash.md`
- `docs/development/consolidated-validation.md`
- `docs/reader/on-device-reader-smoke.md`
- `docs/operations/final-hardware-validation.md`
- `docs/operations/github-upload-checklist.md`

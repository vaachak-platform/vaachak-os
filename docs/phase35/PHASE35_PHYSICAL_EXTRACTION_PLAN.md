...
## Phase 35 Implementation Decision

Phase 35 implements the Vaachak-owned storage state IO seam as compile-time
scaffolding only:
...
The seam maps semantic state kinds to existing Vaachak path helpers. It does
not own SD, SPI, FAT, bookmark, progress, theme, metadata, or reader cache IO.

No imported runtime call site is wired in Phase 35. The state/progress/bookmark
and theme IO call sites are inside imported Pulp reader/kernel internals, so
wiring them would require a later manual decision about how to avoid changing
reader behavior.
...
bookmark/progress/theme persistence
```

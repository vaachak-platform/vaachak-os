...
pub fn resolve_sleep_bitmap(sd: &SdStorage) -> Option<SleepBitmapInfo> {
    ...
    if let Some(info) = probe_sleep_bitmap(sd, DAILY_MANTRA_DEFAULT_BITMAP) {
        return Some(info);
    }
...
pub fn daily_mantra_candidate_from_today_file(sd: &SdStorage) -> Option<SleepBitmapCandidate> {
    ...
    Some(SleepBitmapCandidate::nested("sleep", "daily", weekday_bitmap_name(key)))
}

pub fn probe_sleep_bitmap(sd: &SdStorage, candidate: SleepBitmapCandidate) -> Option<SleepBitmapInfo> {
...
pub fn draw_sleep_bitmap_strip(sd: &SdStorage, info: &SleepBitmapInfo, strip: &mut StripBuffer) -> bool {
...
fn parse_sleep_bitmap_header(candidate: SleepBitmapCandidate, header: &[u8]) -> Option<SleepBitmapInfo> {
...
mod tests {
    use super::{
        SleepBitmapCandidate, WeekdayKey, parse_sleep_bitmap_header, parse_weekday_key,
    };

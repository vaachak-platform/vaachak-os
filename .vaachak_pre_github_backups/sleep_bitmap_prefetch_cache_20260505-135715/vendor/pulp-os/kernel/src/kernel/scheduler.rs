...
impl super::Kernel {
    ...
    async fn render_daily_sleep_bitmap(&mut self) -> bool {
        ...
        esp_println::println!(
            ...
            resolve_start.elapsed().as_millis(),
            bmp_decode_ms.get(),
            sleep_bitmap::sleep_bitmap_cache_hint_for_info(&info)
        );
        ...
        match mode {
            SleepImageMode::FastDaily => {
                self.epd
                    .partial_refresh_async(
                        ...
                        480,
                        &|s: &mut StripBuffer| {
                            if !sleep_bitmap::draw_sleep_bitmap_strip_timed(
                                sd,
                                &info,
                                s,
                                &bmp_read_ms,
            ...
            SleepImageMode::DailyMantra | SleepImageMode::StaticBitmap | SleepImageMode::Cached => {
                self.epd
                    .full_refresh_async(self.strip, &mut self.delay, &|s: &mut StripBuffer| {
                        if !sleep_bitmap::draw_sleep_bitmap_strip_timed(sd, &info, s, &bmp_read_ms)
                        {
                            ok.set(false);
                        }
                    })
        ...
        if ok.get() {
            esp_println::println!(
                "display: sleep bitmap rendered mode={} bmp_read_ms={} bmp_decode_ms={} epd_refresh_ms={} total_ms={}",
                mode.name(),
                bmp_read_ms.get(),
                bmp_decode_ms.get(),
                epd_refresh_ms,
            ...
            esp_println::println!(
                "display: sleep bitmap render failed mode={} bmp_read_ms={} bmp_decode_ms={} epd_refresh_ms={} total_ms={}",
                mode.name(),
                bmp_read_ms.get(),
                bmp_decode_ms.get(),
                epd_refresh_ms,

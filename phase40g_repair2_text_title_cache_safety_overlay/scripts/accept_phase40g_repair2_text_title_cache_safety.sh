...
ROOT="$(pwd)"
DEVICE_OUT="${DEVICE_OUT:-/tmp/phase40g-repair2-device-report.txt}"
OUT="${OUT:-/tmp/phase40g-repair2-text-title-cache-safety-acceptance.txt}"

"$ROOT/phase40g_repair2_text_title_cache_safety_overlay/scripts/check_phase40g_repair2_text_title_cache_safety.sh" >/dev/null
"$ROOT/phase40g_repair2_text_title_cache_safety_overlay/scripts/inspect_phase40g_repair2_text_title_cache_safety.sh" >/dev/null
...
status="ACCEPTED"
reason="TextTitleCacheSafetyAccepted"
...
{
  echo "# Phase 40G Repair 2 Text Title Cache Safety Acceptance"
  echo "status=$status"
  echo "reason=$reason"
  echo "device_status=$device_status"
  echo "old_footer_count=$old_footer_count"
  echo "strict_text_titles=Title-prefix-only"

...
if [[ -e target-xteink-x4/src/vaachak_x4/io/storage_state.rs ]]; then
  ...
  contains "storage state IO seam defines trait or interface" \
    ...
    target-xteink-x4/src/vaachak_x4/io/storage_state.rs
  contains "storage state IO adapter defines path-delegating adapter" \
    'VaachakStorageStateIoAdapter|VaachakStorageStatePathIo' \
...
not_contains "Phase 35 seam and imported wrapper do not move EPUB cache IO" \

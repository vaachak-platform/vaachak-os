...
expected="/tmp/phase26_expected_runtime.rs"
actual="/tmp/phase26_actual_runtime.rs"
diff_file="/tmp/phase26_reader_runtime_sync.diff"
...
if diff -u "$expected" "$actual" > "$diff_file"; then
  echo "OK   Phase 26 imported runtime matches vendored Pulp main after allowed Vaachak marker normalization"
  exit 0
fi

echo "FAIL Phase 26 imported runtime drift detected beyond allowed Vaachak marker normalization"

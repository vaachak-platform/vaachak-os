...
tmp_dir="$(mktemp -d /tmp/phase32_34_imported_runtime_sync.XXXXXX)"
...
cleanup() {
  if [[ "${PHASE32_34_KEEP_DIFF:-0}" != "1" ]]; then
    rm -rf "$tmp_dir"
  fi
}
trap cleanup EXIT
...
if diff -u "$expected" "$actual" > "$diff_file"; then
  echo "OK   Phase 32-34 imported reader runtime matches vendored Pulp main after allowed Vaachak normalization"
  exit 0
fi

echo "FAIL Phase 32-34 imported reader runtime drift detected beyond allowed Vaachak normalization"
echo "Inspect full diff: $diff_file"
echo "Set PHASE32_34_KEEP_DIFF=1 to preserve the temp diff after exit."
echo
head -120 "$diff_file"
exit 1

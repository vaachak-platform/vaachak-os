...
expected="/tmp/phase23_expected_runtime.rs"
actual="/tmp/phase23_actual_runtime.rs"
diff_file="/tmp/phase23_reader_runtime_sync.diff"
...
if diff -u "$expected" "$actual" > "$diff_file"; then
  echo "OK   Phase 23 imported runtime matches vendored Pulp main after allowed Vaachak marker normalization"
  exit 0
fi

echo "FAIL Phase 23 imported runtime drift detected beyond allowed Vaachak marker normalization"

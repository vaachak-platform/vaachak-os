...
expected="/tmp/phase28_expected_runtime.rs"
actual="/tmp/phase28_actual_runtime.rs"
diff_file="/tmp/phase28_reader_runtime_sync.diff"
...
drop_contains = [
    ...
    'BoundaryContractSmoke::emit_boot_marker',
    'boundary_contract_smoke::VaachakBoundaryContractSmoke::emit_boot_marker',
...
if diff -u "$expected" "$actual" > "$diff_file"; then
  echo "OK   Phase 28 imported runtime matches vendored Pulp main after allowed Vaachak marker normalization"
  exit 0
fi

echo "FAIL Phase 28 imported runtime drift detected beyond allowed Vaachak marker normalization"

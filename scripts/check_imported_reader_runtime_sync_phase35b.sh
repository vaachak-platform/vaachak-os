...
expected="/tmp/phase35b_expected_imported_runtime.rs"
actual="/tmp/phase35b_actual_imported_runtime.rs"
diff_file="/tmp/phase35b_imported_reader_runtime_sync.diff"
...
drop_contains = [
    ...
    'display_geometry',
    'spi_bus_runtime',
    'storage_state_runtime',
    'VaachakSpiBusRuntimeBridge',
    'VaachakStorageStateRuntimeBridge',
...
if diff -u "$expected" "$actual" > "$diff_file"; then
  echo "OK   Phase 35B imported reader runtime matches vendored Pulp main after allowed Vaachak normalization"
  exit 0
fi

echo "FAIL Phase 35B imported reader runtime drift detected beyond allowed Vaachak normalization"

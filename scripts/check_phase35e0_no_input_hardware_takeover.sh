...
contains() {
  ...
  local pattern="$2"
  shift 2
  if rg -n -e "$pattern" "$@" >/tmp/phase35e0_hardware_rg.txt 2>/dev/null; then
    ok "$desc"
  else
    fail "$desc"
    printf '      pattern: %s\n' "$pattern"
...
not_contains() {
  ...
  if rg -n -e "$pattern" "$@" >/tmp/phase35e0_hardware_rg.txt 2>/dev/null; then
    fail "$desc"
    cat /tmp/phase35e0_hardware_rg.txt
  else
    ok "$desc"
  fi
}

echo "Phase 35E-0 no input hardware takeover check"
echo
...
not_contains "Vaachak-owned active code does not add physical ADC/debounce ownership" \
  'Adc::new|read_oneshot|decode_ladder|ROW1_THRESHOLDS|ROW2_THRESHOLDS|DEBOUNCE_MS|LONG_PRESS_MS|REPEAT_MS|power_button_is_low' \
...
echo
echo "Phase 35E-0 no input hardware takeover check complete: failures=${failures}"

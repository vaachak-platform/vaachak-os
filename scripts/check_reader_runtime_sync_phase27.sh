...
expected="/tmp/phase27_expected_runtime.rs"
actual="/tmp/phase27_actual_runtime.rs"
diff_file="/tmp/phase27_reader_runtime_sync.diff"
...
drop_contains = [
    ...
    'VaachakBoundaryContract::emit_boot_markers',
    'VaachakStorageStateContractSmoke::emit_boot_marker',
    'VaachakInputContractSmoke::emit_boot_marker',
    'VaachakDisplayContractSmoke::emit_boot_marker',
...
if diff -u "$expected" "$actual" > "$diff_file"; then
  echo "OK   Phase 27 imported runtime matches vendored Pulp main after allowed Vaachak marker normalization"
  exit 0
fi

echo "FAIL Phase 27 imported runtime drift detected beyond allowed Vaachak marker normalization"

...
expected="/tmp/phase20_expected_runtime.rs"
actual="/tmp/phase20_actual_runtime.rs"
diff_file="/tmp/phase20_reader_runtime_sync.diff"
...
if diff -u "$expected" "$actual" > "$diff_file"; then
  echo "OK   Phase 20 imported runtime matches vendored Pulp main after allowed Vaachak marker normalization"
  exit 0
fi

echo "FAIL Phase 20 imported runtime drift detected beyond allowed Vaachak marker normalization"

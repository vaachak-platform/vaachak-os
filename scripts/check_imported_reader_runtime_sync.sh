...
tmp_dir="$(mktemp -d /tmp/phase30_imported_runtime_sync.XXXXXX)"
...
for line in lines:
    ...
    # Allow namespace/module path differences.
...
if diff -u "$expected" "$actual" > "$diff_file"; then
  echo "OK   imported reader runtime matches vendored Pulp main after allowed Vaachak normalization"
  exit 0
fi

echo "FAIL imported reader runtime drift detected beyond allowed Vaachak normalization"
echo "Inspect full diff: $diff_file"
echo "Set PHASE30_KEEP_DIFF=1 to preserve the temp diff after exit."
echo
head -120 "$diff_file"
exit 1

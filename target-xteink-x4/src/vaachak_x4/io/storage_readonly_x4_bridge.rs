...
//! X4-backed read-only implementation bridge for Vaachak storage facade.
//!
...
//! storage helpers in `crate::vaachak_x4::x4_kernel::drivers::storage`.
//!
...
pub const X4_READONLY_BRIDGE_OWNER: &str = "target-xteink-x4 Vaachak adapter bridge";
pub const X4_READONLY_ACTIVE_BACKEND_OWNER: &str = "Vaachak-owned X4 runtime";
pub const X4_READONLY_MAX_DIR_ENTRIES: usize = 32;
pub const X4_READONLY_X4_DIR: &str = "_x4";

pub struct VaachakStorageReadonlyX4BridgeContract;

impl VaachakStorageReadonlyX4BridgeContract {
    ...
    B: X4ReadonlyStorageBackend,
{
    ...
    fn read_start(
        ...
        path: X4ReadonlyPath<'_>,
        out: &mut [u8],
    ) -> Result<VaachakStorageReadChunk, X4ReadonlyStorageBridgeError<B::Error>> {
        if out.is_empty() {
...
impl<B> VaachakReadonlyStorage for X4ReadonlyStorageBridge<B>
where
    B: X4ReadonlyStorageBackend,
{
...
pub struct X4X4ReadonlyStorageBackend<'a> {
    sd: &'a crate::vaachak_x4::x4_kernel::drivers::sdcard::SdStorage,
}

#[cfg(target_arch = "riscv32")]
impl<'a> X4X4ReadonlyStorageBackend<'a> {
    pub const fn new(sd: &'a crate::vaachak_x4::x4_kernel::drivers::sdcard::SdStorage) -> Self {
        Self { sd }
    }

    pub const fn sd(&self) -> &'a crate::vaachak_x4::x4_kernel::drivers::sdcard::SdStorage {
        self.sd
    }
}
...
impl X4ReadonlyStorageBackend for X4X4ReadonlyStorageBackend<'_> {
    type Error = crate::vaachak_x4::x4_kernel::error::Error;

    fn file_size_root(&mut self, name: &str) -> Result<u32, Self::Error> {
        crate::vaachak_x4::x4_kernel::drivers::storage::file_size(self.sd, name)
    }

    fn file_size_in_dir(&mut self, dir: &str, name: &str) -> Result<u32, Self::Error> {
        crate::vaachak_x4::x4_kernel::drivers::storage::file_size_in_dir(self.sd, dir, name)
    }
    ...
    fn file_size_in_subdir(
        ...
        crate::vaachak_x4::x4_kernel::drivers::storage::file_size_in_subdir(
    ...
    fn read_file_start_root(
        ...
        crate::vaachak_x4::x4_kernel::drivers::storage::read_file_start(self.sd, name, out)
    }
    ...
    fn read_file_start_in_dir(
        ...
        crate::vaachak_x4::x4_kernel::drivers::storage::read_file_start_in_dir(
    ...
    fn read_file_start_in_subdir(
        ...
        crate::vaachak_x4::x4_kernel::drivers::storage::read_file_start_in_subdir(
    ...
    fn read_file_chunk_root(
        ...
        crate::vaachak_x4::x4_kernel::drivers::storage::read_file_chunk(self.sd, name, offset, out)
    }
    ...
    fn read_file_chunk_in_dir(
        ...
        crate::vaachak_x4::x4_kernel::drivers::storage::read_file_chunk_in_dir(
    ...
    fn read_file_chunk_in_subdir(
        ...
        crate::vaachak_x4::x4_kernel::drivers::storage::read_file_chunk_in_subdir(
    ...
    fn list_root_entries(
        ...
        let mut raw = [crate::vaachak_x4::x4_kernel::drivers::storage::DirEntry::EMPTY;
            X4_READONLY_MAX_DIR_ENTRIES];
        let limit = core::cmp::min(out.len(), raw.len());
        let count = crate::vaachak_x4::x4_kernel::drivers::storage::list_root_entries(
            self.sd,
            &mut raw[..limit],
        )?;
    ...
    fn list_dir_entries(
        ...
        let mut raw = [crate::vaachak_x4::x4_kernel::drivers::storage::DirEntry::EMPTY;
            X4_READONLY_MAX_DIR_ENTRIES];
        let limit = core::cmp::min(out.len(), raw.len());
        let count = crate::vaachak_x4::x4_kernel::drivers::storage::list_dir_entries(
            self.sd,
            dir,
            &mut raw[..limit],
        )?;
    ...
    fn list_subdir_entries(
        ...
        let mut raw = [crate::vaachak_x4::x4_kernel::drivers::storage::DirEntry::EMPTY;
            X4_READONLY_MAX_DIR_ENTRIES];
        let limit = core::cmp::min(out.len(), raw.len());
        let count = crate::vaachak_x4::x4_kernel::drivers::storage::list_subdir_entries(
            self.sd,
            dir,
            subdir,
            &mut raw[..limit],
...
#[cfg(target_arch = "riscv32")]
fn copy_x4_entries(
    raw: &[crate::vaachak_x4::x4_kernel::drivers::storage::DirEntry],
    out: &mut [X4ReadonlyDirectoryEntry],
) -> Result<usize, crate::vaachak_x4::x4_kernel::error::Error> {
    let mut count = 0usize;
...
mod tests {
    use super::{
        VaachakStorageReadonlyX4BridgeContract, X4ReadonlyDirectoryEntry, X4ReadonlyStorageBackend,
    ...
    fn bridge_preflight_keeps_physical_behavior_imported() {
        let report = VaachakStorageReadonlyX4BridgeContract::active_runtime_preflight();
        assert!(report.ok());
        assert!(!VaachakStorageReadonlyX4BridgeContract::physical_behavior_moved());
    }
}

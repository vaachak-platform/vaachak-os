// Vaachak Wi-Fi transfer server: HTTP file upload + mDNS (x4.local)
...
use crate::vaachak_x4::x4_kernel::kernel::config::WifiConfig;
...
fn v2_file_size(sd: &SdStorage, path: &[u8]) -> crate::vaachak_x4::x4_kernel::error::Result<u32> {
    let (dir, name) = v2_file_parts(path).map_err(|_| {
        crate::vaachak_x4::x4_kernel::error::Error::new(
            crate::vaachak_x4::x4_kernel::error::ErrorKind::InvalidData,
            "v2_file_size_path",
        )
    })?;
...
fn v2_ensure_path(sd: &SdStorage, path: &str) -> crate::vaachak_x4::x4_kernel::error::Result<()> {
...
fn manager_delete_dir(
    ...
    let child_len = compose_child_manager_path(dir, name, &mut child_path).ok_or(
        crate::vaachak_x4::x4_kernel::error::Error::new(
            crate::vaachak_x4::x4_kernel::error::ErrorKind::DeleteFailed,
            "delete_dir_path",
        ),
    )?;

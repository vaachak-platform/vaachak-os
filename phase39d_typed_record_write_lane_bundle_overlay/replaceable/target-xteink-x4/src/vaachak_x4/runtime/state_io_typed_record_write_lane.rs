...
impl Phase39dTypedWriteMode {
    pub const fn commit_requested(self) -> bool {
        matches!(self, Self::CommitToCallback | Self::CommitToRecordingBackend)
    }
}
...
pub fn phase39d_execute_typed_record_write<B: Phase39dTypedWriteBackend>(
    ...
    let path = request.path().unwrap_or_else(Phase39dTypedRecordPath::empty);
...
pub fn phase39d_execute_typed_record_write_with_callback(
    ...
    if backend.callback_bound() {
        phase39d_execute_typed_record_write(Some(&mut backend), request, Phase39dTypedBackendKind::Callback)
    } else {

...
use crate::vaachak_x4::runtime::state_io_home_library_title_patch::{
    phase40g_home_library_title_patch_report, Phase40gNextLane, Phase40gPatchReason,
    Phase40gPatchStatus,
};
...
pub enum Phase40gAcceptanceStatus { Accepted, Rejected }

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum Phase40gAcceptanceReason { PatchAccepted, PatchBlocked }
...
impl Phase40gAcceptanceReport {
    pub const fn accepted(self) -> bool { matches!(self.status, Phase40gAcceptanceStatus::Accepted) }
}
...
pub fn phase40g_acceptance_report() -> Phase40gAcceptanceReport {
    ...
    Phase40gAcceptanceReport {
        status: if report.accepted() { Phase40gAcceptanceStatus::Accepted } else { Phase40gAcceptanceStatus::Rejected },
        reason: if matches!(report.status, Phase40gPatchStatus::Accepted) { Phase40gAcceptanceReason::PatchAccepted } else { Phase40gAcceptanceReason::PatchBlocked },
        patch_reason: report.reason,
        next_lane: report.next_lane,
    }
}

pub fn phase40g_acceptance_marker() -> &'static str { PHASE_40G_HOME_LIBRARY_TITLE_PATCH_ACCEPTANCE_MARKER }
